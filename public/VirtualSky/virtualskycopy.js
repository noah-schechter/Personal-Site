...
stuQuery.prototype.animate = function(end,ms,fn){
	...
	function change(){
		var i,p,v,f;
		var elapsed = 1000000000 * (new Date()) - start;
...
/*
Removed by Noah because this was glitchy
...
function VirtualSky(input){
	...
	this.fps = 60;						// Number of frames per second when animating
	...
	this.colours = {
		...
		'negative':{
			...
			'white':"rgb(255,255,255)",
			'grey':"rgb(100,100,100)",
			'stars':"rgb(46,33,17)",//<- sbrown:rgb(46,33,17)", originally black
			'sun':"rgb(234 88 12)", //<- footnoteorange; originally black
			'moon':'rgb(255,237,213)', //<- orange 100originally black
...
VirtualSky.prototype.resize = function(w,h){
	...
	if(!w || !h){
		if(this.fullscreen){
			...
			this.canvas.width = w*this.getDevicePixelRatio();
			this.canvas.height = h*this.getDevicePixelRatio();
...
VirtualSky.prototype.createSky = function(){
	...
	if(this.c && this.c.getContext){
		...
		ctx.scale(this.getDevicePixelRatio(),this.getDevicePixelRatio());
